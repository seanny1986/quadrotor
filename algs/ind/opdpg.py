...
class OPDPG(nn.Module):
    ...
    def update_policy(self, opt, batch):
        ...
        logprobs = torch.stack(batch.logprob)
        opt.zero_grad()
        actions, lps = self.select_action(states)                                                   # zero gradients in optimizer
        q_loss = self.__critic(torch.cat([states, actions], dim=1))                            # use critic to estimate pol gradient
        ratio = ((lps-logprobs).sum(dim=1,keepdim=True)).exp()
        policy_loss = -(ratio*q_loss).mean()                                            # sum losses
