...
class Actor(nn.Module):
    ...
    def step(self, x, hidden=None):
        #print("x: ", x.unsqueeze(1).size())
        #if hidden: print("hidden: ", hidden[0].size(), hidden[1].size())
        hx, cx = self.__lstm(x.unsqueeze(1), hidden)
        mu = self.__mu(hx.squeeze(1))
        logvar = self.__logvar(hx.squeeze(1))
        return mu, logvar, cx

    def forward(self, x, hidden=None, force=True, steps=0):
        if force or steps == 0: 
            steps = len(x)
        mus = torch.zeros(steps, 1, 1)
        logvars = torch.zeros(steps, 1, 1)
        for i in range(steps):
            if force or i == 0:
                input = x[i]
            mu, logvar, hidden = self.step(input, hidden)
            mus[i] = mu
            logvars[i] = logvar
        return mus, logvars, hidden
...
class TRPO(nn.Module):
    ...
    def select_action(self, state, hidden=None, deterministic=False):
        ...
        """
        mu, logvar, hidden = self.beta.step(state.view(1,-1), hidden)
        if deterministic:
    ...
    def get_value(self, state, hidden=None):
        value, hidden = self.critic.step(state.view(1,-1), hidden)
        return value, hidden
    ...
    def update(self, crit_opt, trajectory):
        def policy_loss(params=None):
            ...
            def get_loss():
                ...
                #print(hxs_a.size())
                avg = 0
                for i in range(1, rewards.size(0)+1):
                    mu_pi, logvar_pi, _ = self.pi.step(states[i-1].unsqueeze(0), (hxs_a[i-1], cxs_a[i-1]))
                    sigma_pi = logvar_pi.exp().sqrt()+1e-10
                    dist = Normal(mu_pi, sigma_pi)
                    log_prob = dist.log_prob(actions[i-1])
                    ratio = (log_prob-fixed_log_probs[i-1].detach()).sum(dim=-1).exp()
                    action_loss = -ratio*advantages[i-1]
                    avg = (avg*(i-1)+action_loss)/i
                return avg
            if params is None:
                return get_loss()
            else:
        ...
        def fvp(vec):
            ...
            for i in range(1, rewards.size(0)+1):
                mu_pi, logvar_pi, _ = self.pi.step(states[i-1].unsqueeze(0), (hxs_a[i-1], cxs_a[i-1]))
                mu_beta, logvar_beta, _ = self.beta.step(states[i-1].unsqueeze(0), (hxs_a[i-1], cxs_a[i-1]))
                ...
                avg = (avg*(i-1)+torch.sum(kl, dim=-1))/i
            kl = avg
        ...
        hidden_as = trajectory["hidden_a"]
        hidden_cs = trajectory["hidden_c"]
        hxs_a = torch.stack([x[0] for x in hidden_as])
        cxs_a = torch.stack([x[1] for x in hidden_as])
        hxs_c = torch.stack([x[0] for x in hidden_cs])
        cxs_c = torch.stack([x[1] for x in hidden_cs])
        ...
        for i in reversed(range(rewards.size(0))):
            if masks[i] == 0:
                #print((hxs_c[i], cxs_c[i]))
                next_val, _ = self.get_value(next_states[i], (hxs_c[i], cxs_c[i]))
                #print("size next val: ", next_val.size())
        ...
        crit_opt.zero_grad()
        crit_loss = F.smooth_l1_loss(values.squeeze(1), returns.detach())
        ...
        #print("policy loss calculated: ", pol_loss)
...
class Trainer:
    def __init__(self, env_name, params, ident=1):
        ...
        if self.__logging:
            filename = self.__directory + "/data/trpo-peb-lstm"+self.__id+"-"+self.__env_name+".csv"
    ...
    def run_algo(self):
        ...
        for ep in range(1, self.__iterations+1):
            s_, a_, ns_, r_, lp_, masks, v_, ha_, hc_ = [], [], [], [], [], [], [], [], []
            num_steps = 1
            reward_batch = 0
            num_episodes = 0
            while num_steps < self.__batch_size+1:
                ...
                while not done:
                    if ep % self.__log_interval == 0 and self.__render:
                        self.__env.render()
                    action, log_prob, hidden_actor = self.__agent.select_action(state, hidden_actor)
                    value, hidden_critic = self.__agent.get_value(state, hidden_critic)
                    next_state, reward, done, _ = self.__env.step(action[0].cpu().data.numpy())
                    reward_sum += reward
                    ...
                    r_.append(reward)
                    lp_.append(log_prob)
                    ha_.append(hidden_actor)
                    hc_.append(hidden_critic)
                    v_.append(value)
            ...
            if (self.__best is None or reward_batch > self.__best) and self.__save:
                ...
                fname = self.__directory + "/saved_policies/trpo-peb-lstm"+self.__id+"-"+self.__env_name+".pth.tar"
        ...
        fname = self.__directory + "/saved_policies/trpo-peb-lstm"+self.__id+"-"+self.__env_name+"-final.pth.tar"
        utils.save(self.__agent, fname)