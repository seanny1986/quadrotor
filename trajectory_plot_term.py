...
def plot_traj(P, A, xyz, zeta, ax, g):
    ...
    q1 = mb.repmat(xyz.T,n+1,1)+q1
    q2 = mb.repmat(xyz.T,n+1,1)+q2
    q3 = mb.repmat(xyz.T,n+1,1)+q3
    q4 = mb.repmat(xyz.T,n+1,1)+q4

    n1 = mb.repmat(xyz.T,n+1,1)+n1
    n2 = mb.repmat(xyz.T,n+1,1)+n2
    n3 = mb.repmat(xyz.T,n+1,1)+n3
    n4 = mb.repmat(xyz.T,n+1,1)+n4
...
def main():
    ...
    directory = os.getcwd()
    fp = directory + "/saved_policies/"+"trpo-1-TrajectoryTerm-v0.pth.tar"
    ...
    while not done:
        ...
        state, reward, done, info = env.step(action.cpu().data.numpy(), term.cpu().item())
