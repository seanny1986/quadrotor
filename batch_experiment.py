...
def main(i, env_name, algs):
    try :
        for alg in algs:
            p = mp.Process(target=make, args=(i, env_name, alg))
            processes.append(p)
            p.start()
...
def make(i, env_name, alg):
    ...
    if alg == "trpo-peb":
        ...
        return trpo_peb.Trainer(env_name, params, "full-spectrum-"+"0.1-"+str(i))
    if alg == "trpo-term":
        params = cfg.trpo
