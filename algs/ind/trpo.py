...
"""
    ...
    to beta, and epsilon is some constant that we choose. DKL is the Kulback-Liebler
    ...
    to do a line search to ensure that  our quadratic approximation is reasonably valid locally,
    and that our update does in fact improve the policy. 
    ...
    order update because it was easier to implement, cleaner, and the algorithm still performed well.
    ...
    4. find grad_{pi} A_{beta}(s,a). Remember, pi and beta are still the same here,
       ...
       cuts down on code when we want to do off-policy correction.
    5. use the conjugate gradient method to find Fv = g, where F is the FIM, and v
       is our estimate of dtheta (starting with grad_{pi} A_{beta}(s,a)).
